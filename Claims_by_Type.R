...
extract_primary_pain_claims <- function(file_path, file_name, pain_dx_codes) {
  ...
  eligibility_stats <- data.table()# Load required libraries
  library(data.table)
  library(haven)
  library(dplyr)
  library(lubridate)
  ...
  in_med_path <- "R:/IQVIA PharMetrics Plus (2024)/Claims - Medical - Inpatient"
  out_med_path <- "R:/IQVIA PharMetrics Plus (2024)/Claims - Medical - Outpatient"
  lookup_path <- "R:/IQVIA PharMetrics Plus (2024)/Lookup Tables"
  mylib_path <- "R:/IQVIA PharMetrics Plus (2024) Members/WatsonWilliam/Pain_Project"
  
  # Load diagnosis lookup table
  cat("Loading diagnosis lookup table...\n")
  dx_lookup <- haven::read_sas(file.path(lookup_path, "pp_dx_lookup.sas7bdat"))
  setDT(dx_lookup)
  cat("Loaded", nrow(dx_lookup), "diagnosis codes\n")
  
  # Define pain diagnosis codes (ICD-10 format)
  pain_diagnoses_icd10 <- c("R52.0", "R52.1", "R52.2", "R52.9", "M54.2", "M54.5", 
                            "M25.50", "M25.51", "M25.52", "M25.53", "M25.54", "M25.55", 
                            "M25.56", "M25.57", "R07.9", "R10.9", "M79.2", "M79.6", 
                            "G89.0", "G89.2", "G89.3", "G89.18", "G89.28")
  
  # Find corresponding dx_cd values for pain diagnoses
  pain_dx_codes <- dx_lookup[diagnosis %in% pain_diagnoses_icd10, dx_cd]
  cat("Found", length(pain_dx_codes), "corresponding dx_cd values for pain diagnoses\n")
  
  # Show the mapping for verification
  pain_dx_mapping <- dx_lookup[diagnosis %in% pain_diagnoses_icd10, .(dx_cd, diagnosis, diagnosis_desc)]
  cat("Pain diagnosis mapping:\n")
  print(pain_dx_mapping)
  ...
  extract_primary_pain_claims <- function(file_path, file_name, pain_dx_codes) {
    ...
    if(file_size_gb > 5) {
      ...
      repeat {
        ...
        # Note: Using %in% with numeric dx_cd values now
        pain_chunk <- chunk[
        ...
        if(nrow(pain_chunk) > 0) {
          ...
          chunk_selected[, `:=`(
            ...
            primary_pain_dx_cd = ifelse(diag_admit %in% pain_dx_codes, diag_admit,
                                        ifelse(diag1 %in% pain_dx_codes, diag1, NA_integer_))
          )]
  ...
  for(file_info in files_to_check) {
    if(file.exists(file_info$path)) {
      results <- extract_primary_pain_claims(file_info$path, file_info$name, pain_dx_codes)
  ...
  # Save results and add diagnosis descriptions
  if(nrow(all_pain_claims_upper) > 0) {
    # Add diagnosis descriptions to the final dataset
    all_pain_claims_upper <- merge(all_pain_claims_upper, 
                                   dx_lookup[, .(dx_cd, diagnosis, diagnosis_desc)], 
                                   by.x = "primary_pain_dx_cd", by.y = "dx_cd", 
                                   all.x = TRUE)
    ...
    fwrite(pain_dx_mapping, file.path(mylib_path, "pain_diagnosis_code_mapping.csv"))
    ...
    if(length(pain_dx_codes) == 0) {
      cat("No matching dx_cd values found for pain diagnoses. Check diagnosis codes in lookup table.\n")
    }
  }
